...
- **`localStorage`-Schreibvorgänge als Seiteneffekt im `setAktiveSB`-Updater:** Alle vier Korrekturrunden an `ActionBoardView.jsx` (W-4 und seine drei Nachbesserungen) drehten sich um dieselbe Grundstruktur -- zwei separat gepflegte `localStorage`-Schlüssel (`dashboard.aktiveSB`, `dashboard.bekannteSB`) plus ein State-Update mit mehreren Zweigen. Der aktuelle Code fasst beide Schlüssel und alle Zweige unter einem gemeinsamen `aktiveKuerzel.length > 0`-Guard zusammen, was die Angriffsfläche verkleinert, aber strukturell nicht ausschließt: ein künftiger dritter Schreibpfad außerhalb dieses Blocks würde denselben Fehlertyp wiederholen.
