...
VALUES (3, 'Modul 9 – Regulierungsverlauf, Kürzungskatalog, Abrechnungsschreiben');
    ...
    # Migration 22: unfalldetails (Schilderung, Zeugen, Ermittlungsakte, Fahrer, Vorsteuer, Haftungsquote)
    22: "-- migration_22_unfalldetails",
    # Migration 23: beteiligte.vertreter_name + vertreter_funktion (für Klage-Rubrum)
    23: "-- migration_23_vertreter",
...
def _run_migration_22(conn: sqlite3.Connection) -> None:
    """
    Migration 22: unfalldetails-Tabelle anlegen.
    Speichert Unfallschilderung, Zeugen, Ermittlungsakte, Fahrer, Vorsteuer, Haftungsquote.
    """
    conn.execute("""
    ...
    conn.execute(
        "INSERT OR IGNORE INTO schema_version (version, beschreibung) "
        "VALUES (22, 'Migration 22 - unfalldetails')"
    )
    logger.info("Migration 22: unfalldetails-Tabelle angelegt.")


def _run_migration_23(conn: sqlite3.Connection) -> None:
    """
    Migration 23: beteiligte um vertreter_name + vertreter_funktion erweitern.
    Für das Klage-Rubrum: GmbH/AG muss mit Vertreter angegeben werden.
    """
    cols = [row[1] for row in conn.execute("PRAGMA table_info(beteiligte)").fetchall()]
    if "vertreter_name" not in cols:
        conn.execute("ALTER TABLE beteiligte ADD COLUMN vertreter_name TEXT")
        logger.info("Migration 23: vertreter_name hinzugefuegt.")
    if "vertreter_funktion" not in cols:
        conn.execute("ALTER TABLE beteiligte ADD COLUMN vertreter_funktion TEXT")
        logger.info("Migration 23: vertreter_funktion hinzugefuegt.")
    conn.execute(
        "INSERT OR IGNORE INTO schema_version (version, beschreibung) "
        "VALUES (23, 'Migration 23 - beteiligte.vertreter_name/funktion')"
    )
    logger.info("Migration 23 abgeschlossen.")
