...
- **Mit Intake-Dokument:** Der Vorgang gilt als abgeschlossen, sobald das letzte Dokument seiner E-Mail-Gruppe die Queue verlässt (freigegeben oder verworfen); die Unfalldaten-Übernahme erfolgt bereits bei der ersten Freigabe auf das erkannte AZ. So behalten Geschwister-Dokumente die AZ-Vorauswahl (Abschnitt 3.4). Bei Freigabe auf das erkannte AZ greift der bestehende Mechanismus (`erstelle_oder_hole_akte`, BUG-08); zusätzlich werden Unfalldatum/Unfallort aus `formular_json` in die Schattenakte übernommen. Wird auf ein **anderes** AZ freigegeben, wird der Vorgang beim Erreichen des letzten offenen Dokuments ebenfalls geschlossen und die UI weist darauf hin, dass die in RA-MICRO angelegte Akte bestehen bleibt (dort ggf. manuell stornieren).
