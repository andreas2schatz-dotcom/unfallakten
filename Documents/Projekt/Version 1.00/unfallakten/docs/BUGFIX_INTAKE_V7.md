...
- **Fix (2026-07-13, Commit `<HASH_P2>`):** `_run_migration_50` auf das Muster der Migrationen 52–55 umgebaut: `conn.executescript(...)` durch einzelne `conn.execute()`-Aufrufe (CREATE TABLE + CREATE INDEX) ersetzt, mit explizitem `conn.commit()` nach den DDLs und um jedes `ALTER TABLE` (davor/danach), plus finales `conn.commit()` nach dem `schema_version`-Insert. **Kein Datenmigrations-Effekt:** Migration 50 ist auf Live-/Dev-DBs (schema_version ≥ 50) bereits eingespielt, ein Re-Run findet nicht statt — reiner Regelkonformitäts-/Fresh-Install-Fix (siehe `feedback_migration_executescript`). Tests: `test_bugfix_p2_intake_v7.py::TestBug13Migration50KeinExecutescript` (3: kein executescript, explizite Commits vorhanden, Verhalten auf frischer DB — Tabelle + Aktivlegit-Spalten).
