...
| BUG-08 | P2 | `backend/routers/intake_routes.py:535` | Freigabe auf RA-MICRO-only-Akten → 404 | **behoben** ✅ |
| BUG-09 | P2 | `backend/services/fristablauf_service.py:134` | SQLite-Selbstblockade im Fristablauf-Job | **behoben** ✅ |
| BUG-10 | P2 | `gunicorn.conf.py:19` | Scheduler laufen unter Gunicorn 4-fach parallel | **behoben** ✅ |
...
### - [x] BUG-08 — Freigabe auf RA-MICRO-only-Akten → 404
- **Fix (2026-07-13, Commit `7b95be7a`):** `post_freigabe` legt eine Akte, die noch nicht in SQLite `unfallakte` existiert (RA-MICRO-only-Kandidat), beim Freigeben **on-demand in SQLite** an — via `models.akte.erstelle_oder_hole_akte(akte_az, bearbeiter_id=…)` (dasselbe Muster wie beim ersten Öffnen einer RA-MICRO-Akte). **RA-MICRO bleibt read-only**, geschrieben wird ausschließlich in SQLite. `benutzer_id` wird dafür vor die Akten-Prüfung gezogen. Reihenfolge bewusst: der BUG-06-Guard (verworfen/bereits freigegeben → 409) läuft **vor** der Anlage, sodass ein verworfenes Dokument keine Akte anlegt. Tests: `test_bugfix_p2_intake_v7.py::TestBug08RamicroOnlyAkte` (2, inkl. Guard-Erhaltung). *(Der aus BUG-01 offene Folge-Task „Fragebogen-Feld-Übernahme" bleibt separat.)*
...
### - [x] BUG-09 — SQLite-Selbstblockade im Fristablauf-Job
- **Fix (2026-07-13, Commit `7b95be7a`):** `verarbeite_faellige_todos` hält die äußere Verbindung nicht mehr über den `schreibe_ereignis`-Aufruf: Todo-Laden, Positions-Lesen und der `UPDATE todos` laufen in **kurzlebigen** Verbindungen (je in eigenem `with get_connection()`), sodass keine unkommittierte Schreibtransaktion den Write-Lock hält, während `schreibe_ereignis` (eigene Verbindung) schreibt. `_erzeuge_fristablauf_fuer_todo` nimmt keinen `conn`-Parameter mehr und öffnet den Positions-Read selbst. Der Ereignis-Write-Guard bleibt unberührt (Schreibpfad weiterhin nur in `ereignis_service.py`). Tests: `test_bugfix_p2_intake_v7.py::TestBug09FristablaufKeinLock` (2 fällige Todos → beide bekommen ihr Ereignis; RED reproduzierte „database is locked" mit 30-s-Wait).
...
### - [x] BUG-10 — Scheduler laufen unter Gunicorn 4-fach parallel
- **Fix (2026-07-13, Commit `7b95be7a`):** Neuer prozessübergreifender **Lease** `backend/services/scheduler_lease.erwirb_scheduler_lease()` — exklusiver TCP-Bind auf einen festen Loopback-Port (`SCHEDULER_LEASE_PORT`, Default 5051). Nur der erste Gunicorn-Worker bekommt den Bind; alle weiteren scheitern mit `EADDRINUSE` und starten **keine** Scheduler (imap_polling/intake_worker/fristablauf). `erstelle_app()` startet die Jobs nur bei `not app.testing and erwirb_scheduler_lease()`. Override `SCHEDULER_LEASE_DISABLED=1` für Einzelprozess-/Dev-Betrieb; `gib_scheduler_lease_frei()` für Tests. Verwandt mit BUG-09 (verhindert die zusätzliche 4-fach-Duplikation der Fristablauf-Ereignisse). Tests: `test_bugfix_p2_intake_v7.py::TestBug10SchedulerLease` (erster Aufruf True, zweiter False).
- **Datei:** `gunicorn.conf.py:19` (+ `backend/app.py`, `erstelle_app()`; neu `backend/services/scheduler_lease.py`)
...
### - [x] BUG-11 — Upload-Validierung (Größe/Typ/PDF-Signatur) umgangen
- **Fix (2026-07-13, Commit `7b95be7a`):** Der Review-Pflicht-Pfad in `dokumente_routes.upload()` ruft jetzt `pdf.upload_service._validiere_datei(filename, bytes)` **vor** `_intake_upload` — dieselben Checks wie der Alt-Pfad (Erweiterungs-Whitelist, Leere-Datei, `MAX_DATEIGROESSE`, PDF-Signatur via `validiere_pdf`). `UploadFehler` → `422` (bzw. deren `status_code`) statt stiller `202`-Ablage in der Queue. Kein neuer Intake-Schreibpfad (Write-Guard unberührt — nur `_validiere_datei` importiert). Tests: `test_bugfix_p2_intake_v7.py::TestBug11UploadValidierung` (5: gefälschtes PDF / leer / verbotene Endung / zu groß → 422, gültiges PDF → 202).
...
### - [x] BUG-12 — O(n²)-OCR: ganzes PDF wird pro Seite neu gerendert
- **Fix (2026-07-13, Commit `7b95be7a`):** `ocr_service.pdf_zu_bildern` bekommt `first_page`/`last_page` (an `convert_from_bytes` durchgereicht). `pipeline._ocr_seite` ruft es mit `first_page=last_page=seite_nr` auf → rendert **nur diese Seite** (Index `[0]`) statt aller Seiten pro Seite. 30-Seiten-Scan: 30 statt 900 Renderings → kein Lease-Ablauf, keine Worker-Doppelung. Tests: `test_bugfix_p2_intake_v7.py::TestBug12OcrLinear` (Mock auf `pdf_zu_bildern` prüft `first_page`/`last_page`-Kwargs + Text).
- **Datei:** `backend/intake/pipeline.py:118` (`_ocr_seite`) + `backend/services/ocr_service.py:105` (`pdf_zu_bildern`)
...
### - [x] BUG-13 — Migration 50 verletzt executescript()-Verbotsregel
- **Fix (2026-07-13, Commit `7b95be7a`):** `_run_migration_50` auf das Muster der Migrationen 52–55 umgebaut: `conn.executescript(...)` durch einzelne `conn.execute()`-Aufrufe (CREATE TABLE + CREATE INDEX) ersetzt, mit explizitem `conn.commit()` nach den DDLs und um jedes `ALTER TABLE` (davor/danach), plus finales `conn.commit()` nach dem `schema_version`-Insert. **Kein Datenmigrations-Effekt:** Migration 50 ist auf Live-/Dev-DBs (schema_version ≥ 50) bereits eingespielt, ein Re-Run findet nicht statt — reiner Regelkonformitäts-/Fresh-Install-Fix (siehe `feedback_migration_executescript`). Tests: `test_bugfix_p2_intake_v7.py::TestBug13Migration50KeinExecutescript` (3: kein executescript, explizite Commits vorhanden, Verhalten auf frischer DB — Tabelle + Aktivlegit-Spalten).
...
- **Fix (2026-07-13, Commit `88271a6a`):** Der Längen-Schwellwert (`body_stripped if len(body_stripped) >= 10 else ""`) in `log_eintrag_meta` entfernt — der bereits `.strip()`-normalisierte Body wird direkt geliefert. Reine Whitespace-Bodies sind durch das `strip()` weiterhin leer, aber legitime Kurz-Antworten („OK, passt"/„Ja") bleiben sichtbar. Tests: `test_bugfix_p3_intake_v7.py::TestBug18KurzerBody` (1: Body „Ja" wird geliefert).
