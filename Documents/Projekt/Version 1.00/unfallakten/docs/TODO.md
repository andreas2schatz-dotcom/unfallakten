...
**Aktueller Schritt:** S1.1 + S1.2 + S1.3 + S1.4 + S1.5 + S1.6a + S1.6b + S1.7 + S1.8 ✅ (2026-07-09 – S1.8: neuer Blueprint `backend/routers/intake_routes.py` (`GET /intake/queue`, `GET /intake/dokument/<id>`, `GET /intake/dokument/<id>/pdf`, `PATCH .../klasse`, `PATCH .../felder`, `POST .../freigabe`), neuer `backend/ramicro/output_adapter.py` (Stufe-1-lokaler Weg via `registriere_dokument`, XML-Scanner folgt), neue Frontend-View `frontend/src/views/ReviewQueueView.jsx` (Queue-Liste + Detail-Panel mit PDF-iframe, Feld-Editor, Klasse-Dropdown, Akten-Kandidaten mit Score-Chip, Zustellungshistorie, LLM/Regex-Diskrepanz-Hinweis) + Route + `apiIntake` in `api.js`. K-2 (Ereignis-Vorschläge) und K-M2b (`ersetzt_ids`) landen als Kontext im `korrektur_log` — Persistierung ins Positionsmodell = P1.5. Freigabe erzeugt `dokumente`-Zeile via output_adapter + `freigaben`-Zeile + setzt `intake_dokumente.queue_status='freigegeben'`. Ohne `akte_az` → 422. 18 neue Tests grün (test_intake_routes.py 12 + test_output_adapter.py 4 + test_s18_review_e2e.py 2). Baseline 211f/524p/0e/18s → 205f/548p/0e/18s (−6 failures, +24 passes, keine Regression). Nächster Schritt: **S1.9** (Umschaltung — BREAKING, Auto-Pfade abschalten + K-P1 Fragebogen-Flow in die Review-Queue).
