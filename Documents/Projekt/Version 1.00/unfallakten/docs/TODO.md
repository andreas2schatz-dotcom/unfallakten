...
**Aktueller Schritt:** **S1 komplett** (S1.1–S1.9) + **P1.1 + P1.2 + P1.3 + P1.4 + P1.5** ✅ (2026-07-09). Nächster Schritt: **P1.6** (System-Ereignisse via APScheduler-Job → `fristablauf`). **Baseline für P1.6**: 206 failed / 657 passed / 0 errors / 18 skipped auf `intake-stufe1` HEAD (Commit 3bae69c). Diffbasierte Regressionschecks bevorzugen (Test-Order-Rauschen bei LM-Studio-abhängigen Tests). Delta gegen P1.4-Baseline (203f/570p): +87 passes durch neue P1.5-Tests (10 Vorbereitung + 7 P1.5a + 7 P1.5b + 5 P1.5c + 5 P1.5d), +3 test-order failures rein in Alt-Kategorien (test_modul1/2/3/4/5/6/7 Auth/Schema, test_dashboard, test_migration_46, test_sv_portal, test_prd27, test_s16a). Keine echte Regression, alle P1-Tests grün.
...
Nächster Schritt: **P1.6** (System-Ereignisse via APScheduler): täglicher Job liest fällige `todos` (quelle=system, erledigt=0, faellig_am ≤ heute) → einmaliges Ereignis `fristablauf` (idempotent über todo-id), Positionen aus dem auslösenden Ereignis (antwort_2w_{dok_id} → Positionen des Dokuments).
