...
**S1 komplett** (S1.1–S1.9). Nächster Schritt gemäß freigabe.md Abschnitt 4: **P1.5** (eingehende Ereignisse ins Positionsmodell, K-M2a): ReguWizard/Beleg-Zuordnung/Gutachten-Übernahme/WDM erzeugen Ereignisse (`abrechnung_eingegangen`/`rechnung_eingegangen`/`gutachten_eingegangen`), Alt-Tabellen parallel weiter befüllen. Testkriterium: nach ReguWizard-Erfassung liefert `/positionen/status` dieselben Beträge wie RegulierungSection.
