...
VHV-Blockformat-Extraktion (`felder.referenzwerkstatt` deterministisch via `extrahiere_verweisbetrieb`), Button „📍 Entfernung prüfen" + Popup in der ReviewQueue (nur `pruefbericht`, nur manuell — ORS-Datenschutz-Entscheidung RA Schatz), Ergebnis-Persistierung (`km_echt`/`bewertung`/`textbaustein` bei unzumutbar), Restbefunde a (Marker-Wortgrenzen) + c (Datums-Scheinkonflikt). 10 Commits `19e9467e..1aa59f79`, 31 BE- + 5 FE-Tests neu, Vollsuite 451/451. Protokoll → `docs/CHANGELOG.md` 2026-08-07. Pläne: `docs/superpowers/plans/2026-08-07-*.md`.
**Offen:** Browser-Abnahme Popup RA Schatz (Achtung: auf 1280/25 kommt aktuell die erwartete 404-Meldung — Mandanten-Adresse lokal leer, siehe Backlog RA-MICRO-Fallback; für die Abnahme entweder Mandant einmal lokal pflegen oder 404-Pfad mit abnehmen). Zurückgestellte Minors (Reparse-Hinweis bei geprüftem Ergebnis, Clipboard-Feedback, Dialog-A11y, 600-Zeichen-Fallback-Test, `VHV_TELEFON_MUSTER`-IGNORECASE) → bei nächster Anfassung der Dateien.
...
### Mittel
- **Entfernungsprüfung: RA-MICRO-Fallback für die Mandanten-Adresse:** `_mandant_adresse` (distanz_routes.py) liest nur die lokale `beteiligte`-Tabelle — bei frischen RA-MICRO-Akten ohne lokalen Workflow-Durchlauf ist die leer (Befund 1280/25: 0 Zeilen) und der Button „Entfernung prüfen" endet im (sauberen) 404-Popup. Read-only-Fallback nach dem Muster `_lade_beteiligte_aus_ramicro` (`word_service.py`, genutzt von beteiligte_/klage_/belege_routes) ergänzen, sonst bleibt das Feature für einen Großteil realer Prüfberichte totes UI. (Final-Review Paket 2, 2026-08-07)
...
| Datum | Feature |
|---|---|
| 2026-08-07 | **Referenzwerkstatt-Extraktion (VHV-Blockformat) + Entfernungsprüfung ReviewQueue (Button+Popup+Persistierung) + Intake-Restbefunde a/c** (Marker-Wortgrenzen, Datums-Scheinkonflikt) — Branch `abschlussbericht`, an Dok 516/517 E2E-verifiziert; Browser-Abnahme offen, siehe „In Arbeit" |
