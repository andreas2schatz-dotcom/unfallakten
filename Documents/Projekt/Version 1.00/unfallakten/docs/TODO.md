...
### Mittel
- **Intake-Klassifikation/Extraktion — Restpunkte aus Befund 1280/25 (2026-08-06):** (a) Marker „Rechnung" trifft als Teilwort auch „**Ab**rechnung" → Wortgrenzen-Matching (\b, vgl. Gutachten-Parser-Lektion), Auto-Vorschlag stufte VHV-Abrechnungsschreiben als `rechnung` ein; (b) ~~Schadennummer-Regex bricht an Leerzeichen ab~~ ✅ gefixt (Runde 2, 2026-08-06); (c) LLM/Regex-Konsens-Check meldet Datums-Scheinkonflikt (ISO vs. deutsch) → vor Vergleich normalisieren. Neu aus Runde 2: (d) `referenzwerkstatt` bleibt leer (Werkstatt-Block außerhalb des N-06-LLM-Seitenfensters). **Nächste Session geplant (2026-08-07, Entscheidungen RA Schatz liegen vor):** (d) deterministisch via `extrahiere_verweisbetrieb`-Erweiterung lösen + Entfernungsprüfung (Button+Popup, anzeigen+speichern) in der ReviewQueue + (a) und (c) mit erledigen — Handover: `handover/naechste_session_referenzwerkstatt_distanz_prompt.md`.
