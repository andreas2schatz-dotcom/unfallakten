...
### 🎯 AKTIV — NÄCHSTE SCHRITTE (Reihenfolge, 2026-07-14)
1. **PROD-ROLLOUT von `intake-stufe1`** (158 Commits vor `main`) — **Runbook: `docs/ROLLOUT-intake-stufe1-prod.md`**. Kernpunkt: Migration 49→57 **einmal vorab** (Gunicorn-4-Worker-Race), Prod-Backup zuerst, Schema-Verifikation vor App-Start. Vom Nutzer als nächster Schritt gesetzt.
2. **Nächstes Hauptfeature: PDF-Splitting im Review-UI (Option C)** — bisher zurückgestellt, jetzt als nächstes Feature gesetzt. Mehrseitige/gemischte PDFs im Review-Dialog in einzelne Dokumente/Klassen auftrennen, bevor freigegeben wird. Start: `superpowers:brainstorming` (UX: wo/wie splitten, Seiten-Auswahl, Klassen-Zuordnung je Teil).
