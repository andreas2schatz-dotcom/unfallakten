...
### Mittel
- **Intake-Klassifikation/Extraktion — Restpunkte aus Befund 1280/25 (2026-08-06):** (a) Marker „Rechnung" trifft als Teilwort auch „**Ab**rechnung" → Wortgrenzen-Matching (\b, vgl. Gutachten-Parser-Lektion), Auto-Vorschlag stufte VHV-Abrechnungsschreiben als `rechnung` ein; (b) ~~Schadennummer-Regex bricht an Leerzeichen ab~~ ✅ gefixt (Runde 2, 2026-08-06); (c) LLM/Regex-Konsens-Check meldet Datums-Scheinkonflikt (ISO vs. deutsch) → vor Vergleich normalisieren. Neu aus Runde 2: (d) `referenzwerkstatt` bleibt leer — der Werkstatt-Block (Möser Arno, Seite 4/5) liegt außerhalb des N-06-Seitenfensters der LLM-Extraktion; bei Bedarf Seitenauswahl um Referenzwerkstatt-Marker erweitern. Kontext: CHANGELOG 2026-08-06.
