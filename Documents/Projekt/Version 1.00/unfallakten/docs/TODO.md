...
- **Nächste Session: N-09 + N-10 + Druckbutton** — Prompt `handover/naechste_session_N09_N10_druckbutton_prompt.md` (vorbereitet 2026-07-13).
- ⚠️ **Prod-Backup ist AKTUELL KAPUTT** (Fund 2026-07-13, Fix in N-10): `docker-compose.prod.yml` mountet `./scripts/backup.sh`, das Skript existiert aber nicht mehr (in `746f731` entfernt) → nächtlicher Backup-Container läuft ins Leere. **Es gibt derzeit kein funktionierendes Backup.** Zusätzlich: `alpine:3.20` hat kein `sqlite3`-Binary. Neu erstellen (mit `.backup` statt `cp`), auf stündlich stellen, Guard-Test gegen Rückfall.
- **ReviewQueue: Druckbutton einfügen** — im ReviewQueueView (Freigabe-/Review-Ansicht) einen Button zum Drucken des aktuell geöffneten Dokuments/der Detailansicht ergänzen. (Wunsch RA Schatz, 2026-07-13.) → Teil der nächsten Session (s. o.).
