...
**Aktueller Schritt:** S1.1 + S1.2 + S1.3 + S1.4 + S1.5 + S1.6a ✅ (2026-07-08 – S1.6a K-P3: Migration 48 (versuch_zaehler/naechster_versuch/fehler_detail/worker_lease auf intake_dokumente), Queue-API mit Lease/Backoff 1/5/30min/MAX_VERSUCHE=3 (backend/intake/queue.py), seitenweise Textgewinnung mit Zeichensalat-Ratio (backend/intake/text_extraktion.py), Tesseract mit image_to_data + TSV-Persistierung (uploads/artefakte/<sha256>/seite_N.tsv), GLM-OCR-Stub hinter Feature-Flag GLM_OCR_ENABLED=false (backend/services/glm_ocr_service.py, F-01), Pipeline-Worker (backend/intake/pipeline.py), APScheduler-Job intake_worker alle 10s single-instance (F-10). 35 neue Tests grün, 0 Regressionen (Baseline unverändert). Nächster Schritt: **S1.6b** (Klassifikator-Kaskade + Extraktion — K-P3-Zusatz Teil 2).
