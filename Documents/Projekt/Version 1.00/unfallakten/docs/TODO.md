...
**Auf ausdrücklichen Wunsch vorgezogen vor P1.5e** (RA Schatz: „nur die zwei zeitkritischen zuerst").
...
Siehe Detailbeschreibung im Abschnitt „Intake-Refactoring" unten (P1.5e). Heute persistiert nur die Gutachten-Freigabe ein echtes Ereignis; alle anderen Klassen landen nur im `korrektur_log`. P1.5e verdrahtet den Freigabe-Dialog mit `ereignis_service.schreibe_ereignis()`. Baseline: 208f/712p Backend + 39 Frontend-Tests. Danach P1.8 (Backfill synthetischer Ereignisse + K-M3) — bzw. N-01–N-06 als Pipeline-Qualität-Session.
