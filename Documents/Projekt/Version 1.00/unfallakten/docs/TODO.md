...
**Aktueller Schritt:** S1.1 + S1.2 + S1.3 + S1.4 + S1.5 + S1.6a + S1.6b + S1.7 ✅ (2026-07-09 – S1.7: neues Modul backend/intake/akten_matching.py mit finde_kandidaten(text, signale) → sortierte Kandidatenliste. Score-Staffel: az_exakt 1.0, az_basis 0.9 (SB-Kürzel entfernt), kfz 0.7, beteiligten_mail 0.6, name_unfalldatum 0.5. Quellen: SQLite (unfallakte, beteiligte) + RA-Micro (read-only, ramicro.email_matching als Wrapper, Fehler ignoriert). Duplikate werden zum höchsten Score reduziert. Pipeline-Integration: Kandidaten landen in parse_json.akten_kandidaten. **KEIN Auto-Zuordnen** von akte_az — die Freigabe-Relation (K-P2) entsteht erst in S1.8. 13 neue Tests (test_intake_akten_matching.py 10 + test_s17_akten_matching_e2e.py 3), isoliert alle grün. Nächster Schritt: **S1.8** (Review-UI-Rohbau, K-2 + K-M2b: Freigabe-Dialog zeigt Ereignis-Vorschläge, „ersetzt"-Auswahl für Ergänzungsgutachten).
