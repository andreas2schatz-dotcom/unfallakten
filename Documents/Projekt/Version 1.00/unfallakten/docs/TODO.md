...
- **Kennzeichen-Erkennung im Akten-Matching zu streng:** `_KFZ_MUSTER` (`backend/intake/akten_matching.py:48`) verlangt zwingend die Schreibweise mit Bindestrich (`OF-BR 1612`). Reale Schreibweisen wie `WÜ PG 777`, `OF A-418`, `OFGM891`, `OF CJ 828` werden nicht erkannt — bei einer Stichprobe von sechs Unfallfragebögen (2026-08-27) traf genau einer das Muster. Für Fragebögen wird das über die strukturierten Bogenfelder umgangen (Spec `2026-08-27-fragebogen-favoritenliste-design.md`); bei gescannten Gutachten, Rechnungen und Schreiben bleibt die Lücke, dort steht das Kennzeichen nur im Volltext. Zu klären: Muster auf freie Schreibweisen erweitern (Trennzeichen optional) und gegen Fehltreffer absichern. Eigenes Vorhaben, klein.
