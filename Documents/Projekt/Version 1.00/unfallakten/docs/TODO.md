...
**Branch `intake-stufe1`, Commits `6863f918`…`3b86f1a9` (nicht gepusht). Spec/Plan: `docs/superpowers/specs/2026-07-11-p15e-freigabe-ereignisse-design.md` + `docs/superpowers/plans/2026-07-12-p15e-freigabe-ereignisse.md`.** Umgesetzt via Subagent-Driven Development (5 Tasks, TDD, Review je Task).
...
- **Baseline nachher:** Backend **204f/730p/18s** (−4f/+18p ggü. Baseline 208f/712p; alle Failures in bekannten Alt-Clustern test_modul3/4/7 etc., **null Failures in P1.5e-berührten Dateien**). Frontend **41 Tests** (8 Files, +2 P1.5e). +21 neue P1.5e/Registry-Tests grün.
- **Bekannter Follow-up (nicht dringend):** `ReviewQueueView.laden()` setzt bei jedem Polling-Tick (nach Reparse, bis 30s) `ereignisse`/`gewaehlteAkte`/`dirty` zurück — bei offenem Freigabe-Dialog während eines Polls können In-Dialog-Bearbeitungen überschrieben werden. Vorbestehendes `laden`-während-Polling-Muster; sauberer Fix (useCallback-Deps) außerhalb P1.5e-Scope. Kandidat für kleines Follow-up-Ticket.
...
**Aktueller Schritt:** **S1 komplett** (S1.1–S1.9) + **P1.1 – P1.7** ✅ (P1.7 am 2026-07-10) + **Review-Erweiterungen** (Reparse-Button, Namens-Fallback, Live-Suche, Restwert netto/brutto, Gutachten+SV-Rechnung Option A, DB-Reparatur) ✅. **Laufend (2026-07-10)**: Review-Bugfixes — Verwerfen-Workflow (Mülltonne + Grund-Dropdown, Migration 53) + Ereignistyp-Dropdown im Freigabe-Dialog (statt JSON-Blob, Registry-Labels). **Nächste Session: P1.5e** — Review-Freigabe schreibt Ereignisse auch für andere Klassen als Gutachten (heute nur Option A). Damit werden die im Freigabe-Dialog gewählten Kandidaten-Ereignisse endlich ins Positionsmodell persistiert statt nur ins korrektur_log. **Danach**: **P1.8** (Backfill synthetischer Ereignisse aus Bestand + K-M3 Backfill-Ehrlichkeits-Hinweis im Dashboard) — auf ausdrücklichen Wunsch zurückgestellt bis P1.5e läuft. **Baseline für P1.5e**: ~217f/682p/18s (nicht kurz vor Handover final verifiziert; P1.7-Baseline 212f/677p + 5 neue Tests aus Erweiterung A alle grün). Zusätzlich Vitest + 32 grüne Frontend-Tests (5 Files, inkl. AktenLiveSuche 6). **Zurückgestellt**: PDF-Splitting im Review-UI (Option C) — kommt nach P1.8, wenn Randfälle konkret werden.

### Offene Bugs / Verbesserungen (nicht dringend)
- Keine offen. (DetailPanel-State-Reset bei Dokument-Wechsel per `key={aktivId}` behoben am 2026-07-10, Commit d4c9cda.)
