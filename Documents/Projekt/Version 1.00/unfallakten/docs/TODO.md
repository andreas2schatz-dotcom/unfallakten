...
**Aktueller Schritt:** S1.1 + S1.2 + S1.3 + S1.4 + S1.5 + S1.6a + S1.6b + S1.7 + S1.8 + **S1.9a** ✅ (2026-07-09). S1.9 wird in Teilschritten a-d umgesetzt, damit der BREAKING-Umbau kontrollierbar bleibt (Rollback-Anker: Feature-Flag).
...
- **S1.9b** — nächster Schritt: import_service.py Schritt 13 (Anhang-Auto-Registrierung) auf `intake_dokumente`+`zustellungen` umstellen; Hash-Dedup → neue Zustellung an bestehendes intake_dokument. Guard-Whitelist reduzieren.
- **S1.9c**: upload_service.py + dokumente_routes.py + eakte_routes.py — Upload und E-Akte-Import erzeugen nur noch Intake-Dokumente. Auto-`setze_schadenpositionen` ≥ 0.85 entfällt.
- **S1.9d**: K-P1 Fragebogen-Parser (Auto-Enrichment `_ergaenze_mandant`/`_gegner`/`_unfalldetails`/`_personenschaden`) durch Review-Queue ersetzen. Frontend `InAkteButton` verlinkt in Review-Queue. Guard-Whitelist leer → Assertion „nur output_adapter schreibt".
