...
### Mittel
- **Übersicht-Redesign („eine Wahrheit pro Information", Review 2026-08-10):** FinanzBand + RegulierungsTabelle raus aus der Übersicht, PositionsDashboard als einzige Positionsansicht, OnboardingHub → Fächer-Chip im PhasenStrip, eine Summen-SSOT (löst Befund B3: Header-KPI rechnet mit HQ, FinanzBand ohne — widersprüchliche Zahlen übereinander). Mockups: `handover/2026-08-10-uebersicht-redesign-mockups.md`. Dabei mit erledigen: 3× `mandant-checks`-Request pro Aktenöffnung, 3× kopierte `dringlichkeit()`-Logik, 2× posMap-Aggregation.
