...
- **Nächste Session (zweite Welle):** N-02 (OCR-Qualitäts-Badge in Review-Queue — nutzt bereits gestempeltes `quote_woerter` + `ratio_salat`, persistiert diese als intake_dokument-Felder), N-03 (Retry-Differenzierung: Timeout/Ressourcendruck/Modellfehler), N-04 (Seiten-Triage/Fotoseiten vor GLM überspringen), N-05 (kooperatives Yielding + Teilergebnisse) + Folge-Task **Fragebogen-Feld-Übernahme bei Freigabe** (aus BUG-01, direkter Alltagsnutzen). Definitionen: `FREIGABE-NACHTRAG-1.md` Abschnitt A. In frischer Session fortsetzen.
