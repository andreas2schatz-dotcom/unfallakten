...
export function tageBadgeText(tage) {
...
export function KachelInhalt({ status, fehlerText, onRetry, retryLaeuft, leer, leerText, children }) {
  if (status === "laedt") {
    return (
      <div aria-hidden="true" style={{ display: "flex", flexDirection: "column", gap: 9, padding: "4px 0" }}>
        {[88, 70, 79].map((breite) => (
  ...
  if (status === "fehler") {
    return (
      <div role="alert" style={{ background: T.redBg, border: `1px solid ${T.redLight}`, borderRadius: 7, padding: "10px 12px" }}>
        <div style={{ fontSize: T.textSm, fontWeight: 600, color: T.redText }}>{fehlerText}</div>
        <button type="button" onClick={onRetry} disabled={!!retryLaeuft} style={{ marginTop: 8, fontSize: T.textXs, fontWeight: 600, color: T.redText, border: `1px solid ${T.redLight}`, background: T.cardBg, borderRadius: 6, padding: "4px 10px", cursor: retryLaeuft ? "default" : "pointer", opacity: retryLaeuft ? 0.5 : 1 }}>
          {retryLaeuft ? "Lädt …" : "Erneut laden"}
        </button>
      </div>
    );
  }
...
export function MehrKnopf({ onClick, children }) {
  return (
    <button type="button" onClick={onClick} style={{ display: "block", marginTop: 9, fontSize: T.textXs, color: T.textMuted, cursor: "pointer", background: "none", border: "none", padding: "4px 2px" }}>
      {children} ›
    </button>
  );
}
