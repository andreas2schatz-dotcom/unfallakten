    ...
    <>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
        ...
        <div style={{ background:T.navyDark, borderRadius:12, padding:"1rem 1.4rem", border:`1px solid ${T.accentTrim}`, display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:T.fontBody, fontSize:"0.945rem", fontWeight:600, color:T.white }}>Dokumente werden automatisch aus den Aktendaten generiert</div>
            <div style={{ fontFamily:T.fontBody, fontSize:"0.875rem", color:"rgba(255,255,255,0.48)", marginTop:2 }}>Kanzlei-Design (Navy/Terrakotta) · DIN 5008 · Haftungsquote {akte.hq} %</div>
          </div>
          <div style={{ display:"flex", gap:16, flexShrink:0 }}>
            {[{l:"Forderung",v:fmtEuro(netto)},{l:"Reguliert",v:fmtEuro(gesamtReg)},{l:"Offen",v:fmtEuro(Math.max(0,netto-gesamtReg))}].map((s,i) => (
              <div key={i} style={{ textAlign:"center" }}>
                ...
                <div style={{ fontFamily:T.fontBody, fontSize:"0.795rem", color:"rgba(255,255,255,0.44)", marginTop:1 }}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            return (
              <Card key={doc.typ} style={{ padding:"1.4rem", display:"flex", flexDirection:"column", gap:14, transition:"transform 0.15s,box-shadow 0.15s" }}
                ...
                <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontFamily:T.fontDisplay, fontSize:"1rem", fontWeight:700, color:T.navy }}>{doc.label}</div>
                    <div style={{ fontFamily:T.fontBody, fontSize:"0.835rem", color:T.textFaint, marginTop:2 }}>An: {doc.an}</div>
                  </div>
                  ...
                  {isD && !err && (
                    <button title={`${doc.label} erneut herunterladen`}
                      ...
                      style={{ flexShrink:0, display:"flex", alignItems:"center", gap:5, padding:"5px 10px", background:T.greenBg, border:`1px solid ${T.green}44`, borderRadius:7, cursor:"pointer", color:T.green, fontFamily:T.fontBody, fontSize:"0.82rem", fontWeight:600, whiteSpace:"nowrap" }}>
                      {Ic.download} .docx
                    </button>
                  )}
                </div>
                ...
                {/* Beschreibung */}
                <p style={{ fontFamily:T.fontBody, fontSize:"0.935rem", color:T.textMuted, lineHeight:1.65, margin:0 }}>{doc.desc}</p>

                {/* Fehlermeldung */}
                {err && (
                  <div style={{ background:T.redBg, border:`1px solid ${T.red}33`, borderRadius:7, padding:"8px 12px", fontFamily:T.fontBody, fontSize:"0.855rem", color:T.red }}>
                    ⚠ {err}
                  </div>
                )}
                ...
                {doc.typ === "forderungsschreiben" && adressatOptionen.length > 0 && (
                  <div>
                    <label style={{ display:"block", fontFamily:T.fontBody, fontSize:"0.8rem", fontWeight:600, color:T.textMuted, marginBottom:5, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                      Adressat
                    </label>
                    <select
                      value={adressatId ?? ""}
                      ...
                      style={{ width:"100%", padding:"7px 10px", borderRadius:7, border:`1.5px solid ${T.border}`,
                        background:T.surface, color:T.text, fontFamily:T.fontBody,
                ...
                <div style={{ marginTop:"auto" }}>
                  <button onClick={() => gen(doc.typ, doc.label)} disabled={isL}
                    style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"9px 14px",
                      ...
                      borderRadius:8, fontFamily:T.fontBody, fontSize:"0.965rem", fontWeight:600,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))", gap:"1.25rem" }}>
          ...
          <Card style={{ padding:"1.4rem", display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:T.fontDisplay, fontSize:"1rem", fontWeight:700, color:T.navy }}>Sachstandsanfrage</div>
                <div style={{ fontFamily:T.fontBody, fontSize:"0.835rem", color:T.textFaint, marginTop:2 }}>Intelligent · Eskalationsstufen</div>
              </div>
            </div>
            ...
            <div style={{ marginTop:"auto" }}>
              <button onClick={() => setStaOffen(true)}
                style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"9px 14px",
                  ...
                  fontFamily:T.fontBody, fontSize:"0.965rem", fontWeight:600,
