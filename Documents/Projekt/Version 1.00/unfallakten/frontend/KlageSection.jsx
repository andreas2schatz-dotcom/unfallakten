...
function KlageSection({ akteId, akte, st, dispatch }) {
  ...
  const oeffneWizard = () => {
    ...
    setWizardPos([...positionen]);   // Kopie, nicht Referenz (B-08)
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"1.75rem",
          ...
          {(st.abrechnungen?.length > 0) && (
            <div style={{ marginTop:"1rem", borderTop:`2px solid ${T.borderSoft}` }}>
              <div style={{ padding:"0.5rem 1.25rem 0.25rem",
                ...
                Regulierungsstand – {st.abrechnungen.length} Abrechnungsschreiben
              </div>
              {/* Abrechnungsschreiben-Liste */}
              <div style={{ padding:"0.25rem 1.25rem 0.5rem",
                borderBottom:`1px solid ${T.borderSoft}` }}>
                {st.abrechnungen.map((ab, i) => (
        ...
        <Card>
          ...
          <div style={{ padding:"0.75rem 1.25rem" }}>
            {/* Streitwert-Split */}
            {(() => {
              // Außergerichtl. SW = alle Positionen (auch vollständig gezahlte)
              const swAusserg = (st.abrechnungen||[]).flatMap(ab => ab.positionen||[])
                .reduce((m, p) => {
                  const k = p.position_key;
                  if (!m[k]) m[k] = 0;
                  m[k] = Math.max(m[k], parseFloat(p.betrag_gefordert)||0);
                  return m;
                }, {});
              const gesamtAusserg = Object.values(swAusserg).reduce((s,v) => s+v, 0);
              const swGericht = klagebetrag; // nur angehakte offene Pos.
